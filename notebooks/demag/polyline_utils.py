...
def create_fillet_segment(a, b, c, r, N):
    ...
    """
    res = find_circle_center_and_tangent_points(a, b, c, r)
    if res is None:
        return [b]
    circle_center, ta, tb = res
    return interpolate_circle(circle_center, ta, tb, N)
...
def create_polyline_fillet(polyline, max_radius, N):
    ...
    for i in range(1, n - 1):
        ...
        if closed and i == n - 2:
            c = filleted_points[1]
        try:
            filleted_points.extend(create_fillet_segment(a, b, c, radius, N))
        except ValueError:
...
def line_plane_intersection(plane_point, plane_normal, line_points, line_directions):
    """
